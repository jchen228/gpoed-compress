...
Scripts are organized into subfolders by role. Data files, generated checkpoints/CSVs/figures stay flat at the repo root regardless of which subfolder produced them (see [Generated artifacts](#generated-artifacts)) — only source files were moved, so run everything from the repo root, e.g.

```bash
python3 progressive_gp/hurricane_progressive_gp.py
```

```
Argonne/
├── core_methods/        # Core Reconstruction Methods
├── libpressio_plugins/  # libpressio Compressor Plugins
├── progressive_gp/      # Progressive GP (Most Recent Method)
├── rate_distortion/     # Rate-Distortion Baseline Comparisons
├── plotting/            # Plotting & Visualization Utilities
├── diagnostics/         # Diagnostics
├── tutorials/           # Tutorials & Paper Replications
├── utils/               # Utilities
```
...
### Core Reconstruction Methods — `core_methods/`

Standalone Kriging/DEIM reconstruction demos. No baseline comparison on their own — see `rate_distortion/` for that.
...
python core_methods/kriging_hurricane.py
```
...
python core_methods/deim_hurricane.py
```
...
```bash
pip install numpy scipy matplotlib
python core_methods/deim_hurricane_v2.py
```
...
```bash
pip install numpy scipy matplotlib scikit-learn
python core_methods/splitgp_hurricane.py
```
...
```bash
pip install numpy scipy matplotlib
python core_methods/tdeim_hurricane.py
```
...
```bash
pip install numpy scipy matplotlib
python core_methods/multigp_hurricane.py
```
...
### libpressio Compressor Plugins — `libpressio_plugins/`

Train/compress/decompress CLI wrappers around the `core_methods/` algorithms, packaged for libpressio's `external:command` protocol.
...
```bash
# 1. Train once:
python3 libpressio_plugins/lp_deim_compressor.py train \
...
python3 libpressio_plugins/lp_deim_compressor.py compress <input.bin> <output.bin> --model /path/to/deim_model.npz

# 3. Decompress:
python3 libpressio_plugins/lp_deim_compressor.py decompress <compressed.bin> <output.bin> --model /path/to/deim_model.npz
```
...
```bash
python3 libpressio_plugins/lp_kriging_compressor.py train \
...
python3 libpressio_plugins/lp_kriging_compressor.py compress <input.bin> <output.bin> --model /path/kriging_model.npz
python3 libpressio_plugins/lp_kriging_compressor.py decompress <compressed.bin> <output.bin> --model /path/kriging_model.npz
```
...
```bash
python3 libpressio_plugins/lp_multigp_compressor.py train \
...
python3 libpressio_plugins/lp_multigp_compressor.py compress <stacked.bin> <output.bin> --model /path/multigp_model.npz
python3 libpressio_plugins/lp_multigp_compressor.py decompress <compressed.bin> <output.bin> --model /path/multigp_model.npz
```
...
```bash
python3 libpressio_plugins/lp_tdeim_compressor.py train \
    --data /path/Uf48.bin.f32 --shape 100 500 500 \
    --model /path/tdeim_model.npz --error-bound 0.01 \
    [--downsample 3] [--oversample 10]

python3 libpressio_plugins/lp_tdeim_compressor.py compress <input.bin> <output.bin> --model /path/tdeim_model.npz
python3 libpressio_plugins/lp_tdeim_compressor.py decompress <compressed.bin> <output.bin> --model /path/tdeim_model.npz
```
...
### Progressive GP (Most Recent Method) — `progressive_gp/`

The actively-evolving round-based sensing method and its SZ3 rate-distortion comparisons, on both the hurricane and SST datasets.
...
```bash
python3 progressive_gp/hurricane_progressive_gp_2d.py
```
...
```bash
python3 progressive_gp/hurricane_gp_comparison.py
```
...
```bash
python3 progressive_gp/sst_progressive_gp.py
```
...
```bash
python3 progressive_gp/sz3_gp_comparison.py
```
...
### Rate-Distortion Baseline Comparisons — `rate_distortion/`

Head-to-head SZ2/ZFP/SZ3 vs. DEIM/Kriging/T-DEIM/MultiGP benchmarks.
...
```bash
python3 rate_distortion/rate_distortion_comparison.py
```
...
```bash
python3 rate_distortion/hurricane_rd_comparison.py
```
...
```bash
python3 rate_distortion/hurricane_rd_3d.py
```
...
```bash
python3 rate_distortion/sst_rd_comparison.py
```
...
### Plotting & Visualization Utilities — `plotting/`

Regenerate figures from already-produced CSVs/results without re-running compression.
...
```bash
python plotting/poster_plots.py            # uses TARGET_CR defined in the config block
TARGET_CR=10 python plotting/poster_plots.py
```
...
```bash
python plotting/poster_plots_wind.py       # uses TARGET_CR defined in the config block
TARGET_CR=10 python plotting/poster_plots_wind.py
```
...
```bash
python3 plotting/plot_timing.py
```
...
```bash
python3 plotting/plot_timing_amortized.py
```
...
```bash
python3 plotting/plot_timing_sst.py
```
...
Analysis-only scripts (no new compression run) that inspect residual structure from existing checkpoints.
...
```bash
python3 diagnostics/lorenzo_vs_global_diagnostic.py
```
...
```bash
python3 diagnostics/residual_autocorr_diagnostic.py
```
...
### Tutorials & Paper Replications — `tutorials/`

libpressio-tutorial exercises and Liang et al. (2018) figure replications. Not part of the main method progression.
...
```bash
mpirun -n 4 python3 tutorials/ex2_comparing_compressors.py
# then plot:
...
```bash
python3 tutorials/q2_zfp_vs_sz.py
```
...
```bash
python3 tutorials/replicate_fig1.py [--bound 5e-4] [--level 50]
```
...
```bash
python3 tutorials/replicate_fig8.py [--target-cr 66] [--level 50]
```
...
One-off inspection/build-fix scripts, unrelated to the reconstruction methods.
...
```bash
python3 utils/mask_hurricane.py
```
...
```bash
python3 utils/patch_libpressio.py
```
